...
pathappend /usr/local/ncbi/blast/bin
...
function awslaunch() {
	...
	ssh -o StrictHostKeyChecking=no -i "~/.ssh/TWM_AWS_ED25519_key1.pem" ubuntu@$(aws ec2 describe-instances --filters Name=instance-id,Values=${instance_id} | awk '$1 == "INSTANCES" {print}' | egrep -o "ec2.*compute.amazonaws.com")
}
...
function awscommand() {
	...
	aws ec2 start-instances --instance-ids ${instance_id} &&
	aws ec2 wait instance-status-ok --instance-ids ${instance_id} &&
	echo "ssh -o StrictHostKeyChecking=no -i \"~/.ssh/TWM_AWS_ED25519_key1.pem\" ubuntu@$(aws ec2 describe-instances --filters Name=instance-id,Values=${instance_id} | awk '$1 == "INSTANCES" {print}' | egrep -o "ec2.*compute.amazonaws.com")"
}
...
function awsvnc() {
	...
	aws ec2 start-instances --instance-ids ${instance_id} &&
	aws ec2 wait instance-status-ok --instance-ids ${instance_id} &&
	address="$(aws ec2 describe-instances --filters Name=instance-id,Values=${instance_id} | awk '$1 == "INSTANCES" {print}' | egrep -o "ec2.*compute.amazonaws.com")" &&
	echo "Launched ${address}"
	ssh -o StrictHostKeyChecking=no -i "~/.ssh/TWM_AWS_ED25519_key1.pem" ubuntu@${address} "vncserver -depth 24 -geometry 1440x900 -localhost :1"
	ssh -i "~/.ssh/TWM_AWS_ED25519_key1.pem" -L 5901:localhost:5901 -C -N ubuntu@${address}
}
