...
function awslaunch() {
	...
	ssh -o StrictHostKeyChecking=no -i "~/.ssh/AWS.pem" ubuntu@$(aws ec2 describe-instances --filters Name=instance-id,Values=${instance_id} | awk '$1 == "INSTANCES" {print $15}')
}
...
function awscommand() {
	...
	aws ec2 start-instances --instance-ids ${instance_id} &&
	aws ec2 wait instance-status-ok --instance-ids ${instance_id} &&
	echo "ssh -o StrictHostKeyChecking=no -i \"~/.ssh/AWS.pem\" ubuntu@$(aws ec2 describe-instances --filters Name=instance-id,Values=${instance_id} | awk '$1 == "INSTANCES" {print $15}')"
}
...
function awsvnc() {
	...
	ssh -o StrictHostKeyChecking=no -i "~/.ssh/AWS.pem" ubuntu@${address} "vncserver -depth 24 -geometry 1440x900 -localhost :1"
	ssh -i "~/.ssh/AWS.pem" -L 5901:localhost:5901 -C -N ubuntu@${address}
}
